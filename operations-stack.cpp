...
int popStack(struct stack *ptr){
    if(isEmpty(ptr)){
        cout<<"The stack is Empty pleas push before pop"<<endl;
        return -1;
    }else{
...
int main(){
    ...
    pushStack(sp, 400);
    pushStack(sp, 10);
    pushStack(sp, 600); //---> only 3 pushes in the stack. at this point stack is full 
    // pushStack(sp, 100); // Stack overflow

    cout<<popStack(sp)<<endl;
    cout<<popStack(sp)<<endl;
    cout<<popStack(sp)<<endl;
    // cout<<popStack(sp)<<endl; // The stack is empty at this point so no other element will be popped out.
